...
type Vehicle = {
  ...
  status: VehicleStatus; created_at: string;
};

type VehicleDocumentHistory = {
  id: string; vehicle_id: string;
  document_type: "rca" | "itp" | "rovinieta" | "casco";
  old_date: string | null; new_date: string | null; created_at: string;
};

type VehicleNote = {
  id: string; vehicle_id: string; note_type: "reparatie" | "observatie";
  title: string | null; content: string; cost: number | null;
  note_date: string | null; created_at: string;
};
...
export default function DetaliuAutoPage() {
  ...
  const loadVehicle = async () => {
    ...
    const [vehicleResponse, documentHistoryResponse, notesResponse] = await Promise.all([
      supabase.from("vehicles").select(`
        id, category, brand, model, registration_number,
        rca_valid_until, rca_cost,
        itp_valid_until, itp_cost,
        has_rovinieta, rovinieta_valid_until, rovinieta_cost,
        has_casco, casco_valid_until, casco_cost,
        is_leasing, monthly_rate, last_rate_date, status, created_at
      `).eq("id", vehicleId).single(),

      supabase.from("vehicle_document_history")
        .select("id, vehicle_id, document_type, old_date, new_date, created_at")
        .eq("vehicle_id", vehicleId).order("created_at", { ascending: false }),

      supabase.from("vehicle_notes")
        .select("id, vehicle_id, note_type, title, content, cost, note_date, created_at")
        .eq("vehicle_id", vehicleId)
        .order("note_date", { ascending: false })
        .order("created_at", { ascending: false }),
    ]);

    if (vehicleResponse.error || !vehicleResponse.data) { router.push("/admin/parc-auto"); return; }

    const v = vehicleResponse.data as Vehicle;
    setVehicle(v);
    ...
    setDocumentHistory((documentHistoryResponse.data as VehicleDocumentHistory[]) || []);
  ...
  const warnings = useMemo(() => {
    ...
    const rovinietaDays = hasRovinieta ? getDaysUntil(rovinietaValidUntil || null) : null;
    const cascoDays = hasCasco ? getDaysUntil(cascoValidUntil || null) : null;
    if (rcaDays !== null && rcaDays >= 0 && rcaDays <= 30) list.push(`Expiră RCA în ${rcaDays} zile`);
    if (itpDays !== null && itpDays >= 0 && itpDays <= 30) list.push(`Expiră ITP în ${itpDays} zile`);
    if (rovinietaDays !== null && rovinietaDays >= 0 && rovinietaDays <= 30) list.push(`Expiră rovinieta în ${rovinietaDays} zile`);
    if (cascoDays !== null && cascoDays >= 0 && cascoDays <= 30) list.push(`Expiră CASCO în ${cascoDays} zile`);
    return list;
  }, [rcaValidUntil, itpValidUntil, hasRovinieta, rovinietaValidUntil, hasCasco, cascoValidUntil, today]);
  ...
  const repairNotes = useMemo(() => vehicleNotes.filter((n) => n.note_type === "reparatie"), [vehicleNotes]);
  const totalRepairCost = useMemo(() => repairNotes.reduce((s, n) => s + Number(n.cost || 0), 0), [repairNotes]);

  const resetRepairFields = () => {
    setRepairTitle(""); setRepairContent(""); setRepairCost("");
    setRepairDate(new Date().toISOString().split("T")[0]);
    setShowRepairNoteForm(false);
  };

  const getDocumentTypeLabel = (type: "rca" | "itp" | "rovinieta" | "casco") => {
  ...
  const handleSave = async () => {
    ...
    if (isLeasing) {
      if (!monthlyRate || Number(monthlyRate) <= 0) { alert("Completează rata lunară."); return; }
      if (!lastRateDate) { alert("Completează data ultimei rate."); return; }
    }
    if (showRepairNoteForm) {
      if (!repairTitle.trim()) { alert("Completează titlul reparației."); return; }
      if (!repairContent.trim()) { alert("Completează observațiile reparației."); return; }
      if (!repairCost || Number(repairCost) < 0) { alert("Completează costul reparației."); return; }
    }
    ...
    const { error: updateError } = await supabase.from("vehicles").update(payload).eq("id", vehicleId);
    if (updateError) { alert(`A apărut o eroare la salvare: ${updateError.message}`); setSaving(false); return; }

    // Document history
    const documentHistoryRows: Array<{ vehicle_id: string; document_type: "rca" | "itp" | "rovinieta" | "casco"; old_date: string | null; new_date: string | null }> = [];

    if ((vehicle.rca_valid_until || null) !== (rcaValidUntil || null)) {
      documentHistoryRows.push({ vehicle_id: vehicleId, document_type: "rca", old_date: vehicle.rca_valid_until || null, new_date: rcaValidUntil || null });
    }
    if ((vehicle.itp_valid_until || null) !== (itpValidUntil || null)) {
      documentHistoryRows.push({ vehicle_id: vehicleId, document_type: "itp", old_date: vehicle.itp_valid_until || null, new_date: itpValidUntil || null });
    }
    if (Boolean(vehicle.has_rovinieta) !== Boolean(hasRovinieta) || (vehicle.rovinieta_valid_until || null) !== (rovinietaValidUntil || null)) {
      documentHistoryRows.push({ vehicle_id: vehicleId, document_type: "rovinieta", old_date: vehicle.has_rovinieta ? vehicle.rovinieta_valid_until || null : null, new_date: hasRovinieta ? rovinietaValidUntil || null : null });
    }
    if (Boolean(vehicle.has_casco) !== Boolean(hasCasco) || (vehicle.casco_valid_until || null) !== (cascoValidUntil || null)) {
      documentHistoryRows.push({ vehicle_id: vehicleId, document_type: "casco", old_date: vehicle.has_casco ? vehicle.casco_valid_until || null : null, new_date: hasCasco ? cascoValidUntil || null : null });
    }

    if (documentHistoryRows.length > 0) {
      const { error: historyError } = await supabase.from("vehicle_document_history").insert(documentHistoryRows);
      if (historyError) alert(`Modificările s-au salvat, dar istoricul nu: ${historyError.message}`);
    }

    if (showRepairNoteForm && repairContent.trim()) {
      const { error: noteError } = await supabase.from("vehicle_notes").insert({
        vehicle_id: vehicleId, note_type: "reparatie", title: repairTitle.trim() || null,
        content: repairContent.trim(), cost: repairCost ? Number(repairCost) : null, note_date: repairDate || null,
      });
      if (noteError) alert(`Modificările s-au salvat, dar nota reparației nu: ${noteError.message}`);
    }
      ...
      <div className="flex min-h-screen flex-col bg-[#F0EEE9]">
        ...
        <div className="flex flex-1 items-center justify-center px-4">
          <div className="flex w-full max-w-xs flex-col items-center gap-5 rounded-[22px] border border-[#E8E5DE] bg-white px-10 py-12 shadow-sm">
            <div className="flex h-14 w-14 items-center justify-center rounded-3xl bg-blue-50"><div className="text-3xl">🚗</div></div>
  ...
  if (!vehicle) return <div className="p-6">Vehiculul nu a fost găsit.</div>;
      ...
      <header className="sticky top-0 z-20 border-b border-[#E8E5DE] bg-white/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => router.push(`/admin/parc-auto/${vehicleId}/edit`)}
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-24 pt-4 sm:px-6 lg:px-8 lg:pb-10">
        ...
        <section className="rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm sm:rounded-[24px] sm:p-6">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-3 flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <h1 className="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">{brand} {model}</h1>
                  <p className="mt-2 text-sm text-gray-500 sm:text-base">
                    Nr. înmatriculare: <span className="font-semibold text-gray-700">{registrationNumber}</span>
                  </p>
                </div>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${computedStatusClasses}`}>{computedStatusLabel}</span>
              {hasRovinieta && <span className="inline-flex rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700">Rovignetă</span>}
              {hasCasco && <span className="inline-flex rounded-full bg-cyan-100 px-3 py-1 text-xs font-semibold text-cyan-700">CASCO</span>}
              {isLeasing && <span className="inline-flex rounded-full bg-purple-100 px-3 py-1 text-xs font-semibold text-purple-700">Leasing</span>}
            </div>
          </div>
          ...
          {warnings.length > 0 && (
            <div className="mt-5 flex flex-wrap gap-2">
              {warnings.map((w) => (
                <span key={w} className="inline-flex rounded-full bg-yellow-100 px-3 py-1 text-xs font-semibold text-yellow-800">{w}</span>
              ))}
            </div>
          )}

          {/* Card documente cu costuri */}
          <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">
            <div className="rounded-2xl bg-[#F8F7F3] px-4 py-4">
              <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Categorie</p>
              <p className="mt-2 text-sm font-bold text-gray-900">{categoryLabelMap[category]}</p>
            </div>

            <div className="rounded-2xl bg-[#F8F7F3] px-4 py-4">
              <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">RCA</p>
              <p className="mt-2 text-sm font-bold text-gray-900">{formatDate(rcaValidUntil)}</p>
              {rcaCost && <p className="mt-1 text-xs font-semibold text-blue-700">{Number(rcaCost).toFixed(2)} RON</p>}
            </div>

            <div className="rounded-2xl bg-[#F8F7F3] px-4 py-4">
              <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">ITP</p>
              <p className="mt-2 text-sm font-bold text-gray-900">{formatDate(itpValidUntil)}</p>
              {itpCost && <p className="mt-1 text-xs font-semibold text-blue-700">{Number(itpCost).toFixed(2)} RON</p>}
            </div>

            <div className="rounded-2xl bg-[#F8F7F3] px-4 py-4">
              <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Rovignetă</p>
              <p className="mt-2 text-sm font-bold text-gray-900">{hasRovinieta ? formatDate(rovinietaValidUntil) : "Nu"}</p>
              {hasRovinieta && rovinietaCost && <p className="mt-1 text-xs font-semibold text-blue-700">{Number(rovinietaCost).toFixed(2)} RON</p>}
            </div>

            <div className="rounded-2xl bg-[#F8F7F3] px-4 py-4">
              <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">CASCO</p>
              <p className="mt-2 text-sm font-bold text-gray-900">{hasCasco ? formatDate(cascoValidUntil) : "Nu"}</p>
              {hasCasco && cascoCost && <p className="mt-1 text-xs font-semibold text-blue-700">{Number(cascoCost).toFixed(2)} RON</p>}
            </div>
          </div>
        </section>
        ...
        {isLeasing && (
          <section className="mt-6 rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm sm:rounded-[24px] sm:p-6">
            <div className="mb-4">
              <h2 className="text-lg font-bold text-gray-900 sm:text-xl">Detalii leasing</h2>
              <p className="mt-1 text-sm text-gray-500">Valori calculate automat în funcție de rata lunară și data ultimei rate.</p>
            </div>
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              <div className="rounded-2xl bg-purple-50 px-4 py-4">
                <p className="text-[11px] uppercase tracking-[0.12em] text-purple-600">Rata lunară</p>
                <p className="mt-2 text-sm font-bold text-purple-900">{Number(monthlyRate || 0).toFixed(2)} lei</p>
              </div>
              <div className="rounded-2xl bg-purple-50 px-4 py-4">
                <p className="text-[11px] uppercase tracking-[0.12em] text-purple-600">Ultima rată</p>
                <p className="mt-2 text-sm font-bold text-purple-900">{formatDate(lastRateDate)}</p>
              </div>
              <div className="rounded-2xl bg-purple-50 px-4 py-4">
                <p className="text-[11px] uppercase tracking-[0.12em] text-purple-600">Luni rămase</p>
                <p className="mt-2 text-sm font-bold text-purple-900">{monthsRemaining}</p>
              </div>
              <div className="rounded-2xl bg-purple-50 px-4 py-4">
                <p className="text-[11px] uppercase tracking-[0.12em] text-purple-600">Rămas de plată</p>
                <p className="mt-2 text-sm font-bold text-purple-900">{remainingLeasingValue.toFixed(2)} lei</p>
              </div>
            </div>
          </section>
        )}
        ...
        <section className="mt-6 grid grid-cols-1 gap-6 xl:grid-cols-2">
          <div className="overflow-hidden rounded-[22px] border border-[#E8E5DE] bg-white shadow-sm">
            <button type="button" onClick={() => setShowDocumentHistory((p) => !p)}
              className="flex w-full items-center justify-between px-5 py-4 text-left transition hover:bg-[#FCFBF8]">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Istoric documente</h2>
                <p className="mt-1 text-sm text-gray-500">Modificările RCA, ITP, rovignetă și CASCO apar aici.</p>
              </div>
            ...
            {showDocumentHistory && (
              <div className="border-t border-[#E8E5DE] p-5">
                {documentHistory.length === 0 ? (
                  <div className="rounded-2xl bg-[#FCFBF8] px-4 py-4 text-sm text-gray-500">Nu există istoric de documente.</div>
                ) : (
                  <div className="space-y-3">
                    {documentHistory.map((item) => (
                      <div key={item.id} className="rounded-2xl border border-[#E8E5DE] bg-[#FCFBF8] px-4 py-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <p className="text-sm font-bold text-gray-900">{getDocumentTypeLabel(item.document_type)}</p>
                            <p className="mt-1 text-sm text-gray-600">{formatDate(item.old_date)} → {formatDate(item.new_date)}</p>
                          </div>
          ...
          <div className="overflow-hidden rounded-[22px] border border-[#E8E5DE] bg-white shadow-sm">
            <button type="button" onClick={() => setShowRepairHistory((p) => !p)}
              className="flex w-full items-center justify-between px-5 py-4 text-left transition hover:bg-[#FCFBF8]">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Istoric reparații</h2>
                <p className="mt-1 text-sm text-gray-500">Toate notele de reparație salvate pentru acest vehicul.</p>
              </div>
              <span className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-[#F8F7F3] text-xl font-semibold text-gray-700">
                {showRepairHistory ? "−" : "+"}
              </span>
              ...
              <div className="border-t border-[#E8E5DE] p-5">
                <div className="mb-4 grid grid-cols-2 gap-3">
                  <div className="rounded-2xl bg-orange-50 px-4 py-4">
                    <p className="text-[11px] uppercase tracking-[0.12em] text-orange-600">Total reparații</p>
                    <p className="mt-2 text-sm font-bold text-orange-900">{repairNotes.length}</p>
                  </div>
                  <div className="rounded-2xl bg-orange-50 px-4 py-4">
                    <p className="text-[11px] uppercase tracking-[0.12em] text-orange-600">Cost total</p>
                    <p className="mt-2 text-sm font-bold text-orange-900">{totalRepairCost.toFixed(2)} lei</p>
                  </div>
                </div>
                {repairNotes.length === 0 ? (
                  <div className="rounded-2xl bg-[#FCFBF8] px-4 py-4 text-sm text-gray-500">Nu există reparații salvate.</div>
                ) : (
                  <div className="space-y-3">
                    {repairNotes.map((note) => (
                      <div key={note.id} className="rounded-2xl border border-[#E8E5DE] bg-[#FCFBF8] px-4 py-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <p className="text-sm font-bold text-gray-900">{note.title || "Reparație"}</p>
                            <p className="mt-1 text-sm text-gray-600">{note.content}</p>
                            <div className="mt-2 flex flex-wrap gap-3 text-xs text-gray-500">
                              <span>Data: {formatDate(note.note_date)}</span>
                              {note.cost != null && <span>Cost: {Number(note.cost).toFixed(2)} lei</span>}
                            </div>
                          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 p-4 pt-6 md:pt-10">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-[24px] border border-[#E8E5DE] bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#E8E5DE] px-5 py-4">
              <div>
                <h2 className="text-lg font-bold text-gray-900">Actualizează auto</h2>
                <p className="text-sm text-gray-500">Modifică datele vehiculului și adaugă reparații.</p>
              </div>
              <button type="button" onClick={() => { setShowEditModal(false); resetRepairFields(); }}
            ...
            <div className="max-h-[78vh] overflow-y-auto px-5 py-5">
              <div className="grid grid-cols-1 gap-6">
                ...
                <div className="rounded-[22px] border border-[#E8E5DE] bg-[#FCFBF8] p-4">
                  <h3 className="text-base font-bold text-gray-900">Informații generale</h3>
                  <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    ...
                    <div>
                      <label className="mb-2 block text-sm font-medium text-gray-700">Status manual</label>
                ...
                {/* Documente cu costuri */}
                <div className="rounded-[22px] border border-[#E8E5DE] bg-[#FCFBF8] p-4">
                  <h3 className="text-base font-bold text-gray-900">Documente și valabilitate</h3>
                  <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                    <div>
                      <label className="mb-2 block text-sm font-medium text-gray-700">RCA valabil până la</label>
                      <input type="date" value={rcaValidUntil} onChange={(e) => setRcaValidUntil(e.target.value)}
                        className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 outline-none focus:border-black" />
                    </div>
                    <div>
                      <label className="mb-2 block text-sm font-medium text-gray-700">ITP valabil până la</label>
                ...
                {/* Leasing */}
                <div className="rounded-[22px] border border-[#E8E5DE] bg-[#FCFBF8] p-4">
                  <h3 className="text-base font-bold text-gray-900">Leasing</h3>
                  <div className="mt-4 rounded-2xl border border-gray-200 bg-white p-4">
                    <label className="flex cursor-pointer items-center gap-3">
                ...
                {/* Nota reparatie */}
                <div className="rounded-[22px] border border-[#E8E5DE] bg-[#FCFBF8] p-4">
                  <button type="button" onClick={() => setShowRepairNoteForm((p) => !p)}
                    className={`w-full rounded-xl px-4 py-3 text-sm font-semibold text-white transition ${showRepairNoteForm ? "bg-gray-500 hover:bg-gray-600" : "bg-[#0196ff] hover:opacity-90"}`}>
                    {showRepairNoteForm ? "Ascunde nota reparație" : "Adaugă notă reparație +"}
                  </button>
                  {showRepairNoteForm && (
                    <div className="mt-4 grid grid-cols-1 gap-4">
                      <div>
                        <label className="mb-2 block text-sm font-medium text-gray-700">Titlu</label>
                        <input type="text" value={repairTitle} onChange={(e) => setRepairTitle(e.target.value)}
                      ...
                      <div>
                        <label className="mb-2 block text-sm font-medium text-gray-700">Data reparației</label>
                      ...
                      <div>
                        ...
                        <textarea value={repairContent} onChange={(e) => setRepairContent(e.target.value)} rows={4}
