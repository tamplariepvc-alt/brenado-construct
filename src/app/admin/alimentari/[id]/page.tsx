...
type FundingDetails = {
  ...
  amount_ron: number | null; funding_type: "card" | "cont";
  funding_date: string; notes: string | null; created_at: string;
};
type ProjectDetails = {
  id: string; name: string; beneficiary: string | null;
  project_location: string | null; status: string; cost_center_code: string | null;
};
...
export default function DetaliuAlimentarePage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const { data: fundingData, error: fundingError } = await supabase
        ...
        .eq("id", fundingId).single();
      if (fundingError || !fundingData) { router.push("/admin/alimentari"); return; }
      const fundingRow = fundingData as FundingDetails;
      const [projectRes, teamLeadRes, adminRes] = await Promise.all([
        supabase.from("projects").select("id, name, beneficiary, project_location, status, cost_center_code").eq("id", fundingRow.project_id).single(),
        supabase.from("profiles").select("id, full_name").eq("id", fundingRow.team_lead_user_id).single(),
        supabase.from("profiles").select("id, full_name").eq("id", fundingRow.added_by).single(),
      ]);
  ...
  const handleExportPdf = () => {
    ...
    const html = `
      <html>
        <head>
          <title>Detaliu alimentare card</title>
          <style>
        ...
        <body>
          ...
          <div class="section">
            ...
            <div class="row"><div class="label">Tip alimentare</div><div class="value">Card</div></div>
  ...
  const handleDelete = async () => {
    setDeleting(true);
    const { error } = await supabase.from("project_fundings").delete().eq("id", fundingId);
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-24 pt-4 sm:px-6 lg:px-8 lg:pb-10">
        <section className="rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm sm:rounded-[24px] sm:p-6">
          <div className="flex items-start justify-between gap-3">
            <div className="flex min-w-0 flex-1 items-start gap-3">
              ...
              <div className="min-w-0">
                <p className="text-sm text-gray-500">Detaliu alimentare card</p>
                <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">{project?.name || "—"}</h1>
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <span className="inline-flex rounded-full bg-green-100 px-3 py-1 text-xs font-semibold text-green-700">Card</span>
                  {project?.status && (
          ...
          <div className="mt-5 flex items-center gap-4">
            <div className="rounded-[20px] bg-[#0196ff] px-6 py-4">
              ...
              <p className="mt-1 text-3xl font-extrabold text-white">{Number(funding.amount_ron || 0).toFixed(2)} lei</p>
            </div>
        ...
        <section className="mt-4 rounded-[22px] border border-[#E8E5DE] bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-3 px-1">
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-gray-400">Date proiect</p>
            <div className="h-px flex-1 bg-[#E8E5DE]" />
          </div>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-4">
            {[
              { label: "Proiect", value: project?.name || "-" },
              { label: "Cod centru cost", value: project?.cost_center_code || "-" },
              { label: "Beneficiar", value: project?.beneficiary || "-" },
              { label: "Locație", value: project?.project_location || "-" },
              { label: "Șef șantier", value: teamLead?.full_name || "-" },
              { label: "Alimentat de", value: adminProfile?.full_name || "-" },
            ].map(({ label, value }) => (
              <div key={label}>
        ...
        <section className="mt-4 rounded-[22px] border border-[#E8E5DE] bg-white p-5 shadow-sm">
          <div className="mb-4 flex items-center gap-3 px-1">
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-gray-400">Date alimentare</p>
            <div className="h-px flex-1 bg-[#E8E5DE]" />
          </div>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div>
              <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Tip</p>
              <p className="mt-1 text-sm font-semibold text-gray-900">Card</p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Sumă</p>
              <p className="mt-1 text-sm font-semibold text-gray-900">{Number(funding.amount_ron || 0).toFixed(2)} lei</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="mt-1 text-sm font-semibold text-gray-900">
                {funding.created_at ? new Date(funding.created_at).toLocaleDateString("ro-RO") : "-"}
              </p>
            </div>
          </div>
          {funding.notes && (
            <div className="mt-5">
              ...
              <div className="rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-700">{funding.notes}</div>
            </div>
          )}
        </section>
      </main>
          ...
          <div className="w-full max-w-sm overflow-hidden rounded-[24px] border border-[#E8E5DE] bg-white shadow-2xl">
            <div className="p-6">
              ...
              <p className="mt-2 text-sm text-gray-500">
                Ești sigur că vrei să ștergi alimentarea de <span className="font-semibold text-gray-800">{Number(funding.amount_ron || 0).toFixed(2)} lei</span> pentru <span className="font-semibold text-gray-800">{project?.name || "-"}</span>? Această acțiune nu poate fi anulată.
              </p>
            </div>
