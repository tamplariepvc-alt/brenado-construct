...
export default function OreExtraWeekendPage() {
  ...
  const loadData = async () => {
    ...
    const { data: extraData, error: extraError } = await supabase
      .from("extra_work")
      .select(`
        id, project_id, worker_id, work_date,
        extra_hours, is_saturday, is_sunday,
        extra_hours_paid, weekend_paid,
        extra_hours_value, weekend_days_count,
        weekend_value, total_value, created_at
      `)
  ...
  const workerMap = useMemo(
    () => new Map(workers.map((w) => [w.id, w.full_name])),
    [workers]
  );
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const isExtra =
        Number(row.extra_hours || 0) > 0 || Number(row.extra_hours_value || 0) > 0;
      const isWeekend =
      ...
      if (workerSearch.trim() && !workerName.includes(workerSearch.trim().toLowerCase()))
        return false;
      if (projectSearch.trim() && !projectName.includes(projectSearch.trim().toLowerCase()))
        return false;
  ...
  const exportPdf = () => {
    ...
    const periodLabel =
      periodFilter === "doua_saptamani" ? "Ultimele 2 saptamani"
      : periodFilter === "zi" ? `Zi selectata: ${selectedDay || "-"}`
      : periodFilter === "interval" ? `Interval: ${startDate || "-"} - ${endDate || "-"}`
      : "Toate";
    const selectedProjectLabel =
      selectedProjectId === "placeholder" ? "Alege santier"
      : selectedProjectId === "all" ? "Toate santierele"
      : projectMap.get(selectedProjectId) || "-";
    const paymentLabel =
      paymentFilter === "toate" ? "Toate"
      : paymentFilter === "achitate" ? "Achitate"
      : "Neachitate";
    ...
    if (category === "extra") {
      ...
      autoTable(doc, {
        ...
        body: filteredRows.map((row) => [
          getWorkerName(row), getProjectName(row),
      ...
      autoTable(doc, {
        startY: 56,
        head: [["Nume", "Santier", "Data", "Sambata", "Duminica", "Zile", "Valoare", "Status"]],
        body: filteredRows.map((row) => [
          getWorkerName(row), getProjectName(row),
          new Date(row.work_date).toLocaleDateString("ro-RO"),
          row.is_saturday ? "Da" : "Nu",
          row.is_sunday ? "Da" : "Nu",
          Number(row.weekend_days_count || 0).toFixed(0),
  ...
  const activeDays = category === "extra" ? totals.extraHours : totals.weekendDays;
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm text-gray-500 sm:text-base">
                Vizualizează și gestionează orele suplimentare și zilele de weekend lucrate.
              </p>
            </div>
          </div>
        ...
        <section className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className={`rounded-[20px] p-4 shadow-sm ${category === "extra" ? "bg-purple-600" : "bg-orange-500"}`}>
            <p className="text-xs font-semibold text-white/80">
              {category === "extra" ? "Total ore" : "Total zile"}
            </p>
            <p className="mt-1 text-2xl font-extrabold text-white">
              {category === "extra"
                ? `${totals.extraHours.toFixed(1)} h`
                : `${totals.weekendDays} zile`}
            </p>
          </div>
          ...
          <div className="rounded-[20px] border border-[#E8E5DE] bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-2xl font-extrabold text-red-600">
              {filteredRows.filter((r) =>
                category === "extra" ? !r.extra_hours_paid : !r.weekend_paid
              ).length}
            </p>
          </div>
        </section>
                  ...
                  >
                      ...
                      {category === "extra" ? (
                        <>
                          <div className="rounded-2xl bg-purple-50 px-4 py-3">
                            ...
                            <p className="mt-0.5 text-lg font-bold text-purple-800">
                              {Number(row.extra_hours || 0).toFixed(1)} h
                            </p>
                          </div>
                          <div className="rounded-2xl bg-purple-50 px-4 py-3">
                        ...
                        <>
                          ...
                          <div className="rounded-2xl bg-orange-50 px-4 py-3">
                            ...
                            <p className="mt-0.5 text-lg font-bold text-orange-700">
                              {Number(row.weekend_days_count || 0)} zile
                            </p>
                          </div>
                          <div className="rounded-2xl bg-orange-50 px-4 py-3">
                    ...
                    {!isPaid && (
                      <button
                        ...
                        onClick={() =>
                          category === "extra"
                            ? handleMarkExtraPaid(row)
                            : handleMarkWeekendPaid(row)
                        }
                        disabled={isProcessing}
                        className={`mt-4 w-full rounded-xl px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60 ${
                          category === "extra" ? "bg-purple-600" : "bg-orange-500"
                        }`}
                      >
                        {isProcessing
                          ? "Se procesează..."
