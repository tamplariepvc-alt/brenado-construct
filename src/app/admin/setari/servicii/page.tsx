...
export default function SetariServiciiPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F0EEE9]">
                ...
                {filteredServices.map((svc) => (
                  <div key={svc.id} className="grid grid-cols-12 items-center border-b border-[#E8E5DE] px-5 py-4 last:border-b-0">
                    ...
                    <div className="col-span-2 flex justify-end gap-2">
                      ...
                      <button type="button" onClick={() => setDeleteConfirmId(svc.id)} disabled={deleting === svc.id}
      ...
      {deleteConfirmId && (() => {
        const svc = services.find((s) => s.id === deleteConfirmId);
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="w-full max-w-sm overflow-hidden rounded-[24px] border border-[#E8E5DE] bg-white shadow-2xl">
              <div className="p-6">
                ...
                <h3 className="text-center text-lg font-bold text-gray-900">Ștergi acest serviciu?</h3>
                <p className="mt-2 text-center text-sm text-gray-500">Acțiunea este ireversibilă.</p>
                {svc && (
                  <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3">
                    <p className="text-sm font-bold text-red-800">{svc.name}</p>
                    <p className="mt-1 text-xs text-red-600">{svc.um} · {Number(svc.price_ron).toFixed(2)} lei/{svc.um}</p>
                  </div>
                )}
