...
export default function SetariPage() {
  ...
  const openEdit = (svc: Service) => {
    ...
    setName(svc.name);
    setUm(svc.um);
    setPrice(String(svc.price_ron));
    setIsActive(svc.is_active);
    setShowForm(true);
  };

  const handleSave = async () => {
    if (!name.trim()) { alert("Completează denumirea serviciului."); return; }
    if (!um.trim()) { alert("Completează unitatea de măsură."); return; }
    if (!price || Number(price) < 0) { alert("Completează un preț valid."); return; }

    setSaving(true);

    if (editingId) {
      const { error } = await supabase.from("services").update({
        name: name.trim(), um: um.trim(),
        price_ron: Number(price), is_active: isActive,
      }).eq("id", editingId);
  ...
  const filteredServices = services.filter((s) =>
    ...
    s.um.toLowerCase().includes(search.trim().toLowerCase())
  );
    ...
    <div className="min-h-screen bg-[#F0EEE9]">
      <header className="sticky top-0 z-20 border-b border-[#E8E5DE] bg-white/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-3">
            <button onClick={() => router.push("/admin")} className="rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">
              Înapoi la admin
            </button>
            <button onClick={openAdd} className="rounded-xl bg-[#0196ff] px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90">
              + Serviciu nou
            </button>
          </div>
        </div>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-10 pt-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm text-gray-500 sm:text-base">
                Gestionează serviciile, materialele și alte configurări ale aplicației.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-6">
          <div className="mb-4 rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm sm:p-6">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h2 className="text-lg font-extrabold text-gray-900">Servicii deviz</h2>
                <p className="mt-1 text-sm text-gray-500">
                  Serviciile din care șefii de echipă completează devizul zilnic. Ei văd denumirea și UM, prețul e vizibil doar pentru administrator.
                </p>
              </div>
            ...
            <div className="mt-4">
              <input
                type="text"
                placeholder="Caută după denumire sau UM..."
                value={search}
          ...
          {/* Form adauga / editeaza */}
          {showForm && (
            <div className="mb-4 rounded-[22px] border border-[#0196ff]/30 bg-white p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                  <label className="mb-2 block text-sm font-semibold text-gray-700">
                    Preț (lei / UM) <span className="text-red-500">*</span>
                  </label>
          ...
          {filteredServices.length === 0 ? (
            <div className="rounded-[22px] border border-[#E8E5DE] bg-white p-6 shadow-sm">
              <p className="text-sm text-gray-500">
                {search ? "Nu există servicii pentru căutarea introdusă." : "Nu există servicii adăugate. Adaugă primul serviciu."}
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="hidden overflow-hidden rounded-[22px] border border-[#E8E5DE] bg-white shadow-sm lg:block">
                <div className="grid grid-cols-12 border-b border-[#E8E5DE] bg-[#F8F7F3] px-5 py-4 text-sm font-semibold text-gray-700">
                  <div className="col-span-5">Denumire serviciu</div>
                    ...
                    <div className="col-span-1">
                      <button
                        ...
                        className={`rounded-full px-2.5 py-1 text-xs font-semibold transition ${
                          svc.is_active ? "bg-green-100 text-green-700 hover:bg-green-200" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                        }`}
                      >
                  ...
                  <div key={svc.id} className="rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="mt-0.5 text-sm text-gray-500">UM: {svc.um}</p>
                        <p className="mt-1 text-sm font-bold text-gray-900">{Number(svc.price_ron).toFixed(2)} lei / {svc.um}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => handleToggleActive(svc)}
                    ...
                    <div className="mt-3 flex gap-2">
                      <button
                        type="button"
                        onClick={() => openEdit(svc)}
                        className="flex-1 rounded-xl border border-gray-200 bg-white py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50"
                      >
                        Editează
                      </button>
                      <button
                        ...
                        onClick={() => handleDelete(svc.id)}
                        disabled={deleting === svc.id}
                        className="flex-1 rounded-xl border border-red-200 bg-red-50 py-2 text-sm font-semibold text-red-600 transition hover:bg-red-100 disabled:opacity-60"
                      >
                        {deleting === svc.id ? "..." : "Șterge"}
                      </button>
                    </div>
        ...
        {/* Placeholder pentru sectiuni viitoare */}
        <section className="mt-6">
          <div className="rounded-[22px] border border-dashed border-[#E8E5DE] bg-white/50 p-6 shadow-sm">
            <p className="text-center text-sm text-gray-400">
              Alte secțiuni (materiale comenzi, configurări etc.) vor fi adăugate aici.
            </p>
          </div>
        </section>
      </main>
