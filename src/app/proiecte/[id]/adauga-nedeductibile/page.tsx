...
export default function AdaugaNedeductibilePage() {
  ...
  useEffect(() => {
    const loadProject = async () => {
      ...
      if (error || !data) { router.push("/proiecte"); return; }
  ...
  const handleSave = async () => {
    ...
    if (!expenseDate) { alert("Completeaza data."); return; }
    if (!serviceName.trim()) { alert("Completeaza serviciul."); return; }
    if (!costRon || Number(costRon) <= 0) { alert("Completeaza costul serviciului."); return; }
  ...
  if (loading) return <div className="p-6">Se incarca proiectul...</div>;
  if (!project) return <div className="p-6">Proiectul nu a fost gasit.</div>;
      ...
      <div className="mx-auto max-w-3xl">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600">Proiect: <span className="font-semibold">{project.name}</span></p>
          </div>
        ...
        <div className="rounded-2xl bg-white p-5 shadow">
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700">Serviciu</label>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700">Cost serviciu (RON)</label>
              <input type="number" step="0.01" min="0" value={costRon} onChange={(e) => setCostRon(e.target.value)}
                placeholder="Ex: 250"
                className="w-full rounded-lg border border-gray-300 px-4 py-3 outline-none focus:border-black" />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700">Observatii</label>
