...
type DailyReportItem = { id: string; daily_report_id: string; service_id: string; quantity: number };
...
function PhotoLightbox({ photos, initialIndex, onClose }: {
  ...
  const prev = useCallback(() => setCurrent((c) => (c > 0 ? c - 1 : photos.length - 1)), [photos.length]);
  const next = useCallback(() => setCurrent((c) => (c < photos.length - 1 ? c + 1 : 0)), [photos.length]);
    ...
    >
      ...
      {photos.length > 1 && photos.length <= 20 && (
        <div className="absolute bottom-14 left-1/2 flex -translate-x-1/2 gap-1.5">
          {photos.map((_, i) => (
            <button key={i} type="button" onClick={(e) => { e.stopPropagation(); setCurrent(i); }}
...
export default function ProiectePage() {
  ...
  const closeLightbox = useCallback(() => { setLightboxPhotos([]); setLightboxIndex(0); }, []);
  ...
  const loadData = async () => {
    ...
    if (profileData.role === "administrator") {
      const { data } = await supabase.from("projects")
        .select("id, name, beneficiary, status, created_at").order("created_at", { ascending: true });
    ...
    if (projectIds.length > 0) {
      const fundingsQuery = profileData.role === "sef_echipa"
        ...
        : supabase.from("project_fundings").select("project_id, amount_ron").in("project_id", projectIds);
      ...
      const [fundingsRes, receiptsRes, invoicesRes, nondeductiblesRes, servicesRes,
        ...
        supabase.from("daily_reports").select("id, project_id, report_date, created_by")
        ...
        supabase.from("daily_photos").select("id, project_id, photo_date, photo_url, uploaded_by")
  ...
  const setProjectTab = (id: string, tab: ProjectTab) => setProjectTabs((prev) => ({ ...prev, [id]: tab }));
  const setTehnicTab = (id: string, tab: TehnicTab) => setTehnicTabs((prev) => ({ ...prev, [id]: tab }));
  ...
  const resetInlineState = (projectId: string, type: UploadDocType) => {
    setActiveInlineProjectId(projectId); setActiveInlineType(type);
  ...
  const applyExtractedData = (data: any) => {
    setFormState({
      documentDate: data.document_date || "", supplier: data.supplier || "",
  ...
  const handlePhotoUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of Array.from(files)) {
      ...
      await supabase.from("daily_photos").insert({
        project_id: photoProjectId, photo_date: today,
        photo_url: urlData.publicUrl, uploaded_by: user.id,
      });
    }
  ...
  const handleSaveDeviz = async () => {
    ...
    if (itemsError) { alert(`Eroare: ${itemsError.message}`); setSavingDeviz(false); return; }
                  ...
                  <div key={project.id} className="overflow-hidden rounded-[22px] border border-[#E8E5DE] bg-white shadow-sm">
                    <div className="p-4 sm:p-5">
                      ...
                      <div className="mt-5 flex gap-2">
                        <button type="button" onClick={() => setProjectTab(project.id, "financiar")}
                          className={`flex-1 rounded-2xl py-2.5 text-sm font-semibold transition ${projectTab === "financiar" ? "bg-[#0196ff] text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                          Financiar
                        </button>
                        <button type="button" onClick={() => setProjectTab(project.id, "tehnic")}
                          className={`flex-1 rounded-2xl py-2.5 text-sm font-semibold transition ${projectTab === "tehnic" ? "bg-gray-900 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                          Tehnic
                        </button>
                      </div>
                      ...
                      {projectTab === "tehnic" && (
                        <div className="mt-4">
                          <div className="flex gap-2">
                            <button type="button" onClick={() => setTehnicTab(project.id, "poze")}
                              className={`flex-1 rounded-2xl py-2 text-sm font-semibold transition ${tehnicTab === "poze" ? "bg-amber-500 text-white" : "bg-amber-50 text-amber-700 hover:bg-amber-100"}`}>
                              Poze
                            </button>
                            <button type="button"
                              <button
    ...
    setDevizProjectId(null); // 🔴 NU deschide formularul automat
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {allPhotoDates.map((date) => (
                                      <button key={date} type="button"
                                        ...
                                        className={`rounded-xl px-3 py-1.5 text-xs font-semibold transition ${photoViewDateForProject === date ? "bg-amber-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                              ...
                              {photosForDate.length > 0 && (
                                <div>
                                  <p className="mb-2 text-xs font-semibold text-gray-500">
                                    ...
                                    {isAdmin && <span className="ml-2 text-gray-400">· Apasă pe o poză pentru a o mări · Swipe stânga/dreapta pentru navigare</span>}
                                  </p>
                                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                                    {photosForDate.map((photo, idx) => (
                                      <button key={photo.id} type="button"
                                        ...
                                        <img src={photo.photo_url} alt="Poză șantier"
                          ...
                          {tehnicTab === "deviz" && (
                            <div className="mt-4 space-y-4">
                                                  ...
                                                  <div key={item.id} className="flex items-center justify-between gap-2 text-xs">
                                                    ...
                                                    {isAdmin && (
                                                      <span className="font-medium text-gray-900">
                                                        {((svc?.price_ron || 0) * item.quantity).toFixed(2)} lei
                                                      </span>
                                                    )}
                                                  </div>
                                                );
                              ...
                              {!isAdmin && isDevizOpen && (
                                <div className="rounded-2xl border border-teal-200 bg-teal-50 p-4">
                                  <div className="mb-4 flex items-center justify-between gap-3">
                                    ...
                                    <input type="date" value={devizDate} onChange={(e) => setDevizDate(e.target.value)}
                                          ...
                                          <div className="flex items-start gap-2">
                                            <div className="flex-1 space-y-2">
                                              <select value={line.service_id}
                                                ...
                                                {services.map((svc) => (
                                                  <option key={svc.id} value={svc.id}>{svc.name} ({svc.um})</option>
                                                ))}
                                              </select>
                                              <div className="flex items-center gap-2">
                                  ...
                                  <div className="mt-4 flex gap-3">
                                    <button type="button" onClick={addDevizLine}
                                    ...
                                    <button type="button" onClick={handleSaveDeviz} disabled={savingDeviz}
                                    ...
                                    <button type="button" onClick={() => setDevizProjectId(null)}
                              ...
                              {/* Butoane creare — doar sef_echipa */}
                              {!isAdmin && !isDevizOpen && projectReports.length === 0 && (
                                <div>
                                  <p className="mb-3 text-sm text-gray-400">Nu există devize pentru acest proiect.</p>
                                  <button type="button" onClick={() => openDeviz(project.id)}
                    ...
                    {/* Financiar inline form — doar sef_echipa */}
