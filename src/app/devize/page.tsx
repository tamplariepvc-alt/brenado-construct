    ...
    const loadData = async () => {
      ...
      const [estimatesRes, itemsRes, servicesRes] = await Promise.all([
        supabase.from("estimates")
          ...
          .eq("created_by", profileData.role === "administrator" ? profileData.id : profileData.id)
      ...
      const { data: estimatesData } = await estimatesQuery;
                            ...
                            {estimateItems.slice(0, 5).map((item) => {
                              ...
                              return (
                                <div key={item.id} className="flex items-center justify-between gap-2 text-xs">
                                  <span className="min-w-0 flex-1 truncate text-gray-700">{service?.name || "-"}</span>
