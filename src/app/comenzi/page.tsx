...
type StatusFilterKey = "toate" | "asteapta_confirmare" | "aprobata" | "refuzata";
...
export default function ComenziPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // Roluri permise: admin + variante + project_manager + admin_limitat + sef_echipa
      ...
      // Fetch comenzi — admin/cont_tehnic/project_manager văd toate, restul doar ale lor
      const isFullAccess = ["administrator", "cont_tehnic", "project_manager", "admin_limitat"].includes(profileData.role);

      let query = supabase
        .from("orders")
        .select(`
          id, order_number, project_id, order_date, status,
          total_with_vat, created_by, created_at,
      ...
      if (ordersError || !ordersData) { setLoading(false); return; }
      ...
      let normalizedOrders: OrderRow[] = typedOrdersFromDb.map((order) => ({
        ...
        projects: order.projects?.[0] ? { name: order.projects[0].name } : null,
      }));

      // Fallback fetch proiecte fara nume
      const missingProjectIds = Array.from(new Set(
        normalizedOrders.filter((o) => !o.projects?.name && o.project_id).map((o) => o.project_id)
      ));

      if (missingProjectIds.length > 0) {
        const { data: projectsData } = await supabase
          .from("projects").select("id, name").in("id", missingProjectIds);

        if (projectsData && projectsData.length > 0) {
          const projectMap: Record<string, string> = {};
          projectsData.forEach((p: { id: string; name: string }) => { projectMap[p.id] = p.name; });
          normalizedOrders = normalizedOrders.map((order) => {
            if (!order.projects?.name && projectMap[order.project_id]) {
              return { ...order, projects: { name: projectMap[order.project_id] } };
            }
            return order;
          });
        }
      ...
      const userIds = Array.from(new Set(normalizedOrders.map((o) => o.created_by)));
      if (userIds.length > 0) {
        const { data: profilesData, error: profilesError } = await supabase
          .from("profiles").select("id, full_name").in("id", userIds);
        if (!profilesError && profilesData) {
          const namesMap: ProfileNameMap = {};
          profilesData.forEach((p) => { namesMap[p.id] = p.full_name; });
  ...
  const canCreate = !isAdminLimitat; // admin_limitat nu poate crea/edita/aproba
  const isFullAdmin = ["administrator", "cont_tehnic"].includes(profile?.role || "");
  const canApprove = ["administrator", "cont_tehnic", "project_manager"].includes(profile?.role || "");

  const filteredOrders = useMemo(() => {
    return orders.filter((order) => {
      const matchesStatus = statusFilter === "toate" || order.status === statusFilter;
      const projectName = order.projects?.name || "";
      const matchesSantier = projectName.toLowerCase().includes(searchSantier.toLowerCase());
  ...
  const stats = useMemo(() => ({
    total: orders.length,
    inAsteptare: orders.filter((o) => o.status === "asteapta_confirmare").length,
    aprobate: orders.filter((o) => o.status === "aprobata").length,
    refuzate: orders.filter((o) => o.status === "refuzata").length,
  }), [orders]);
  ...
  const getFilterCardClasses = (filter: StatusFilterKey) => {
    ...
    if (filter === "toate") return active ? "border-blue-500 ring-2 ring-blue-200 bg-blue-50" : "border-transparent bg-blue-50";
    if (filter === "asteapta_confirmare") return active ? "border-amber-400 ring-2 ring-amber-200 bg-amber-50" : "border-transparent bg-amber-50";
    if (filter === "aprobata") return active ? "border-green-400 ring-2 ring-green-200 bg-green-50" : "border-transparent bg-green-50";
    return active ? "border-red-400 ring-2 ring-red-200 bg-red-50" : "border-transparent bg-red-50";
  };

  const getPageSubtitle = () => {
    if (isAdminLimitat) return "Vizualizare comenzi";
    if (isFullAdmin || profile?.role === "project_manager") return "Administrare comenzi";
    return "Comenzile tale";
  };

  const getPageDesc = () => {
    if (isAdminLimitat) return "Vizualizezi comenzile din sistem (doar citire).";
    if (isFullAdmin || profile?.role === "project_manager") return "Vezi și gestionează toate comenzile din sistem.";
    return "Vezi comenzile create de tine.";
  };
      ...
      <header className="sticky top-0 z-20 border-b border-[#E8E5DE] bg-white/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex gap-3">
            ...
            {/* Buton Adaugă — ascuns pentru admin_limitat */}
            {canCreate && (
              <button onClick={() => router.push("/comenzi/adauga")}
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-24 pt-4 sm:px-6 lg:px-8 lg:pb-10">
        <section className="rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm sm:rounded-[24px] sm:p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">Comenzi</h1>
              <p className="mt-3 max-w-2xl text-sm text-gray-500 sm:text-base">{getPageDesc()}</p>
              {/* Badge readonly pentru admin_limitat */}
              {isAdminLimitat && (
          ...
          <div className="mt-5 grid grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-3">
            {(["toate", "asteapta_confirmare", "aprobata", "refuzata"] as StatusFilterKey[]).map((filter) => {
              const configs = {
                toate: { val: stats.total, label: "Toate", color: "blue" },
                asteapta_confirmare: { val: stats.inAsteptare, label: "În așteptare", color: "amber" },
                aprobata: { val: stats.aprobate, label: "Aprobate", color: "green" },
                refuzata: { val: stats.refuzate, label: "Refuzate", color: "red" },
              };
              const cfg = configs[filter];
              return (
                <button key={filter} type="button" onClick={() => setStatusFilter(filter)}
                  className={`rounded-2xl border px-3 py-3 text-center transition ${getFilterCardClasses(filter)}`}>
                  <p className={`text-2xl font-extrabold tracking-tight text-${cfg.color}-600 sm:text-3xl`}>{cfg.val}</p>
                  <p className={`mt-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-${cfg.color}-300`}>{cfg.label}</p>
                </button>
              );
            })}
          </div>
          ...
          <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
            <input type="text" placeholder="Caută după șantier" value={searchSantier}
        ...
        <section className="mt-6">
          <div className="mb-3 flex items-center gap-3 px-1">
            <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-gray-400">Lista comenzi</p>
            <div className="h-px flex-1 bg-[#E8E5DE]" />
          </div>

          {filteredOrders.length === 0 ? (
            <div className="rounded-[22px] border border-[#E8E5DE] bg-white p-5 shadow-sm">
              <p className="text-sm text-gray-500">Nu există comenzi pentru filtrele selectate.</p>
            </div>
          ) : (
            <>
              {/* Desktop table */}
                ...
                {filteredOrders.map((order, index) => (
                  <button key={order.id}
                    ...
                    <div className="col-span-2 text-sm font-semibold text-gray-900">
                      {order.order_number ? order.order_number.replace("CMD-", "") : String(index + 1).padStart(4, "0")}
                    </div>
                    <div className="col-span-3 text-sm text-gray-600">
                      {order.projects?.name || <span className="italic text-gray-400">Fără șantier</span>}
                    </div>
                ...
                {filteredOrders.map((order, index) => (
                  <button key={order.id}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className={`inline-block shrink-0 rounded-full px-2.5 py-1 text-xs font-semibold ${getStatusColor(order.status)}`}>
                        {getStatusLabel(order.status)}
                      </span>
                    </div>
                    <div className="mt-4 grid grid-cols-2 gap-3 pr-10">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-gray-700">{new Date(order.order_date).toLocaleDateString("ro-RO")}</p>
                      </div>
                      <div>
                        <p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Valoare</p>
                        <p className="mt-1 text-sm font-bold text-gray-900">{Number(order.total_with_vat || 0).toFixed(2)} lei</p>
                      </div>
                    </div>
