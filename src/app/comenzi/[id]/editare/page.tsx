...
export default function EditareComandaPage() {
  ...
  const toastColors: Record<ToastType, string> = {
    ...
    success: "border-[#0196ff]/30 bg-[#0196ff]/10 text-[#0057b3]",
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (orderData.status !== "draft") { router.push(`/comenzi/${orderId}`); return; }
      if (profileData.role === "sef_echipa" && orderData.created_by !== user.id) {
  ...
  const subtotal = useMemo(() =>
    items.reduce((sum, item) => sum + item.unit_price * (Number(item.quantity) || 0), 0), [items]);
  const vatTotal = useMemo(() =>
    items.reduce((sum, item) => sum + item.unit_price * (Number(item.quantity) || 0) * (item.vat_percent / 100), 0), [items]);
    ...
    <div className="min-h-screen bg-[#F0EEE9]">
      ...
      <header className="sticky top-0 z-20 border-b border-[#E8E5DE] bg-white/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-3 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <button onClick={() => router.push(`/comenzi/${orderId}`)}
        ...
        <section className="rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm sm:rounded-[24px] sm:p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-3 max-w-2xl text-sm text-gray-500 sm:text-base">Modifică articolele și trimite comanda când este gata.</p>
            </div>
          </div>
        </section>
        ...
        <div className="mt-6 space-y-6">
          <section className="rounded-[22px] border border-[#E8E5DE] bg-white p-5 shadow-sm">
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-500">Actualizează datele generale ale draftului.</p>
            </div>
            ...
            <div className="mt-4">
              ...
              <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
                placeholder="Adaugă observații pentru comandă" rows={3}
                className="w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 outline-none transition focus:border-black" />
            </div>
          </section>
            ...
            <div className="flex items-center justify-between border-b border-[#E8E5DE] px-5 py-4">
              <div>
                ...
                <p className="mt-0.5 text-sm font-bold text-gray-900">
                  {items.filter(i => i.article_id).length} {items.filter(i => i.article_id).length === 1 ? "articol" : "articole"}
                </p>
              </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <div className="flex flex-col">
              <button type="button" onClick={handleSaveDraft} disabled={savingDraft || sendingOrder}
            ...
            <button type="button" onClick={handleSendOrder} disabled={savingDraft || sendingOrder}
