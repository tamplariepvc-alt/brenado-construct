...
type OrderDetails = {
  id: string; order_number: string | null; project_id: string; created_by: string;
  order_date: string; status: string; subtotal: number; vat_total: number;
  total_with_vat: number; notes: string | null; created_at: string;
  creator_name?: string; projects?: { name: string } | null;
};
...
type OrderItem = {
  ...
  article_name: string; unit: string | null; unit_price: number; quantity: number;
  line_total: number; vat_percent: number; line_total_with_vat: number;
};
...
export default function ComandaDetaliuPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setOrder({
        id: typedOrderData.id, order_number: typedOrderData.order_number,
        project_id: typedOrderData.project_id, created_by: typedOrderData.created_by,
        order_date: typedOrderData.order_date, status: typedOrderData.status,
        subtotal: typedOrderData.subtotal, vat_total: typedOrderData.vat_total,
        total_with_vat: typedOrderData.total_with_vat, notes: typedOrderData.notes,
        created_at: typedOrderData.created_at, creator_name: creatorProfile?.full_name || "-",
  ...
  // Permisiuni derivate
  const canApprove = ["administrator", "cont_tehnic", "project_manager"].includes(profile?.role || "");
  const canEdit = ["administrator", "cont_tehnic", "project_manager", "sef_echipa"].includes(profile?.role || "");
  ...
  const handleExportPdf = async () => {
    ...
    await exportOrderPdf({
      orderNumber: order.order_number || "-", projectName: order.projects?.name || "-",
      orderDate: new Date(order.order_date).toLocaleDateString("ro-RO"),
      creatorName: order.creator_name || "-", status: order.status,
      subtotal: Number(order.subtotal || 0), vatTotal: Number(order.vat_total || 0),
      totalWithVat: Number(order.total_with_vat || 0),
      items: items.map((item) => ({
        article_code: item.article_code, article_name: item.article_name,
        unit: item.unit, quantity: Number(item.quantity || 0),
            ...
            <div className="p-6">
              <div className={`mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full ${confirmModal.action === "aprobata" ? "bg-green-100" : "bg-red-100"}`}>
                {confirmModal.action === "aprobata" ? (
                  <svg viewBox="0 0 24 24" fill="none" className="h-7 w-7 text-green-600" stroke="currentColor" strokeWidth="2"><path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                ) : (
                  <svg viewBox="0 0 24 24" fill="none" className="h-7 w-7 text-red-600" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" strokeLinecap="round" strokeLinejoin="round" /></svg>
                )}
              </div>
              ...
              <p className="mt-2 text-center text-sm text-gray-500">
                {confirmModal.action === "aprobata" ? "Comanda va fi marcată ca aprobată și procesată." : "Comanda va fi marcată ca refuzată. Această acțiune nu poate fi anulată."}
              </p>
              ...
              <div className="mt-5 flex flex-col gap-3 sm:flex-row">
                <button type="button" onClick={() => updateOrderStatus(confirmModal.action)} disabled={actionLoading}
                  className={`flex-1 rounded-xl px-5 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60 ${confirmModal.action === "aprobata" ? "bg-green-600" : "bg-red-600"}`}>
                  {actionLoading ? "Se procesează..." : confirmModal.action === "aprobata" ? "Da, confirmă" : "Da, refuză"}
                </button>
                <button type="button" onClick={() => setConfirmModal(null)} disabled={actionLoading}
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <h1 className="mt-1 text-2xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">{order.order_number || "Comandă fără număr"}</h1>
                  <p className="mt-1 text-sm font-medium text-gray-400">{order.projects?.name || "-"}</p>
                </div>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <span className={`inline-flex w-fit rounded-full px-3 py-1 text-xs font-semibold ${statusClasses}`}>{statusLabel}</span>
              {isAdminLimitat && (
                <span className="inline-flex rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-500">Doar vizualizare</span>
              )}
            </div>
          </div>

          <div className="mt-5 grid grid-cols-2 gap-3 xl:grid-cols-4">
            <div><p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Nr. comandă</p><p className="mt-1 text-sm font-semibold text-gray-900">{order.order_number || "-"}</p></div>
            <div><p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Șantier</p><p className="mt-1 text-sm font-semibold text-gray-900">{order.projects?.name || "-"}</p></div>
            <div><p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Data comenzii</p><p className="mt-1 text-sm font-semibold text-gray-900">{new Date(order.order_date).toLocaleDateString("ro-RO")}</p></div>
            <div><p className="text-[11px] uppercase tracking-[0.12em] text-gray-400">Creată de</p><p className="mt-1 text-sm font-semibold text-gray-900">{order.creator_name || "-"}</p></div>
          </div>
        ...
        <section className="mt-4 rounded-[22px] border border-[#E8E5DE] bg-white p-4 shadow-sm sm:p-5">
          <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
            <button onClick={handleExportPdf}
              ...
              <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" stroke="currentColor" strokeWidth="2"><path d="M12 16V4M7 11l5 5 5-5" strokeLinecap="round" strokeLinejoin="round" /><path d="M4 20h16" strokeLinecap="round" /></svg>
              Export PDF
            </button>

            {/* Editează — draft + canEdit + nu admin_limitat */}
            {order.status === "draft" && canEdit && !isAdminLimitat && (
              <button type="button" onClick={() => router.push(`/comenzi/${order.id}/editare`)}
                ...
                <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" strokeLinecap="round" strokeLinejoin="round" /><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" strokeLinecap="round" strokeLinejoin="round" /></svg>
                Editează
              </button>
            )}

            {/* Refuză + Confirmă — canApprove + asteapta_confirmare */}
            {canApprove && order.status === "asteapta_confirmare" && (
              <>
                <button type="button" onClick={() => setConfirmModal({ action: "refuzata" })} disabled={actionLoading}
                  ...
                  <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" stroke="currentColor" strokeWidth="2"><path d="M18 6L6 18M6 6l12 12" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  Refuză
                </button>
                <button type="button" onClick={() => setConfirmModal({ action: "aprobata" })} disabled={actionLoading}
                  ...
                  <svg viewBox="0 0 24 24" fill="none" className="h-4 w-4" stroke="currentColor" strokeWidth="2"><path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                  Confirmă
                </button>
              </>
            )}
            ...
            <>
              <div className="hidden overflow-hidden lg:block">
                <div className="grid grid-cols-12 border-b bg-[#F8F7F3] px-4 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500">
                  <div className="col-span-1">Nr.</div><div className="col-span-2">Cod</div>
                  <div className="col-span-4">Denumire</div><div className="col-span-1">U.M.</div>
                  <div className="col-span-1">Qty</div><div className="col-span-2">P.U.</div>
                ...
                {items.map((item, index) => (
                  <div key={item.id} className="px-4 py-3">
                    <div className="flex items-start gap-2">
                      <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-gray-100 text-[10px] font-bold text-gray-500">{index + 1}</span>
                    ...
                    <div className="mt-2 flex items-center justify-between pl-7">
                      <div className="flex items-center gap-2 text-xs text-gray-500">
                        <span>{item.article_code || "-"}</span><span>·</span>
                        <span>{Number(item.quantity).toFixed(0)} {item.unit || ""}</span><span>·</span>
                        <span>{Number(item.unit_price).toFixed(2)} lei</span>
                      </div>
